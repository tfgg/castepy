...
if __name__ == "__main__":
  ...
  for line in f:
    ...
    name = line_s[0]
    isotope, symbol = re.findall("([0-9]+)([A-Za-z]+)", line_s[1])[0]
    isotope = int(isotope)

    if (symbol, isotope) not in g or g[(symbol, isotope)] is None:
      print name, (symbol, isotope)

  for symbol, iso in Q_iso.items():
    if (symbol, iso) not in Q:
      print "Q missing:", symbol, iso
